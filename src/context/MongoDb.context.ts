...
class MongodbContext {
  ...
  private async getEmbedding(input: IEmbeddingInput) {
    return this.embedding.getEmbedding(input.query, input.additionalInfo.userIdentifier);
  }


  protected async query(input: IQueryInput, projection: Document): Promise<Document[]> {
    ...
    const documents = await this.collectionInstance.aggregate(aggregatePipeline).toArray();
  ...
  async addReference(input: IAddDocumentInput): Promise<void> {
    ...
    if (!result.acknowledged) {
      ...
      console.error(`Successfully insert a reference. for customerId: ${customerId}`);
    }
  }
}
